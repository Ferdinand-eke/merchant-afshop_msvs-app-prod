...
function PhotosVideosTab(props) {
  ...
  const {
    data: martMenu,
    isLoading: martMenuLoading,
    error,
  } = useMyShopFoodMartMenus(foodMart?.id);
    ...
    return (
      <div className="flex flex-1 items-center justify-center h-full">
        <MerchantShopClientErrorPage
          message={`Error occured while retrieving menu`}
        />
      </div>
    );
  }
            ...
            <div className="mb-48">
              <div className="overflow-hidden flex flex-row flex-wrap -m-8">
                {martMenu?.data?.foodmartmenus?.map((menu) => (
                  <div className="w-full sm:w-1/2 md:w-1/4 p-8" key={menu.id}>
                    <ImageListItem
                      component={motion.div}
                      variants={item}
                    ...
                    >
                      {menu.imageSrc ? (
                        <img src={menu.imageSrc} alt={menu.title} height={60} />
                      ) : (
                        <img
