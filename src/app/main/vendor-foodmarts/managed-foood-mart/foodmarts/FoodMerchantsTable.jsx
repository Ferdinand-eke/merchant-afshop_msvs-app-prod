...
function FoodMerchantsTable() {
  const { data: foodMartData, isLoading: foodMartIsLoading } =
    useMyShopFoodMarts();
...

      {
        ...
        Cell: ({ row }) => (
          <div className="flex flex-wrap space-x-2">
            <Chip
              component={Link}
              to={`/foodmarts/management-portal/${row.original.slug}/manage`}
  ...
  if (!foodMartData?.data?.foodmarts) {
    return (
  ...
  return (
    <Paper
      ...
      elevation={0}
    >
      <DataTable data={foodMartData?.data?.foodmarts} columns={columns} />
    </Paper>
  );
}
