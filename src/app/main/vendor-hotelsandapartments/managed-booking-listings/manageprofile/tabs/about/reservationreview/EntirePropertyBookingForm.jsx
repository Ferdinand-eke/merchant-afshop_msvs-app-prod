...
/**
 * EntirePropertyBookingForm - Component for merchants to book entire property for walk-in guests
 * Features:
 ...
 * - Booking submission for entire property
 */
...
const EntirePropertyBookingForm = ({ propertyId, propertyPrice, propertyTitle, propertyAddress, merchantId, onCreateReservation, isCreatingReservation = false, onClose }) => {
	...
	const [totalPrice, setTotalPrice] = useState(propertyPrice);
	...
	const { data: reservationsData, isLoading: reservationsLoading } = useFetchReservationsOnProperty(propertyId);

	// Calculate disabled dates from existing reservations
	const disabledDates = useMemo(() => {
		let dates = [];
		reservationsData?.data?.reservations?.forEach((reservation) => {
	...
	useEffect(() => {
		if (dateRange?.startDate && dateRange?.endDate) {
			...
			if (dayCount && propertyPrice) {
				setTotalPrice(dayCount * propertyPrice);
			} else {
				setTotalPrice(propertyPrice);
			}
		}
	}, [dateRange, propertyPrice]);
	...
	}, [formData, dateRange, totalPrice, propertyId, propertyTitle, propertyAddress, merchantId, identityCardImage, onCreateReservation]);
			...
			content={
				<Box sx={{ p: 3, maxWidth: 500 }}>
					...
					<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
						<Box>
							<Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
								<Home sx={{ color: '#ea580c', fontSize: 24 }} />
					...
					<Alert severity="info" sx={{ mb: 3 }}>
						You are booking the <strong>entire property</strong> for a walk-in guest. All rooms will be reserved for this booking period.
					</Alert>
					...
					<Paper elevation={0} sx={{ p: 3, mb: 3, backgroundColor: '#fafafa', border: '1px solid #e5e7eb' }}>
						...
						<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
							...
							<TextField
								...
								placeholder="Special requests, group details, etc."
					...
					<Alert severity="warning" sx={{ mb: 2 }}>
						This booking will reserve the <strong>entire property</strong> for the selected dates. The guest identity card has been captured for security verification as required by hospitality regulations.
					</Alert>
				</Box>
			}
		/>
...
export default EntirePropertyBookingForm;
