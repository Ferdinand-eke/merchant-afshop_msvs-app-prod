...
function CloseAccountSetting(props) {
	...
	const { control, setError, reset, watch, handleSubmit, formState, getValues } = useForm({
		defaultValues,
		mode: 'all',
	...
	const onCloseAccount = () => {
		if (getValues().shopemail?.toString() === getValues()?.checkEmail?.toString()) {
			if (
				window.confirm(
					'Are you sure abou this?. We would love to have you reconsider, how do we help avoid this, because this action is irreversible as all data concerning you will be totally erased from our services'
				)
			) {
		...
		<div className="w-full max-w-3xl">
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<div className="w-full">
					<Typography className="text-xl">Close Account preferences</Typography>
					<Typography color="text.secondary">
						Close your account if this you consider taking a break from us with following preferences.
					</Typography>
				</div>
									...
									<Controller
										...
										render={({ field }) => (
											<div className="flex flex-col w-full">
												...
												<FormHelperText>
													Please type the email as seen in the above field here to really
													confirm your decision on closing this account
												</FormHelperText>
											</div>
										)}
									/>
