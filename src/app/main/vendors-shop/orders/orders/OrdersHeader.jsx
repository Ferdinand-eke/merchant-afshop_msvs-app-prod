	...
	return (
		<div className="flex space-y-12 sm:space-y-0 flex-1 w-full items-center justify-between py-8 sm:py-16 px-16 md:px-24">
			...
			<div className="flex flex-1 sm:w-auto  items-center justify-end space-x-8">
				...
				>
					<EarningsTab shopDataProps={shopData?.data?.data} />
				</motion.div>
			</div>
		</div>
	);
...
function EarningsTab({ shopDataProps }) {
	...
	return (
		...
		>
			...
			{shopDataProps?.shopaccount?.accountbalance && (
				<span className="mx-4 sm:mx-8">
					Earnings: NGN {formatCurrency(shopDataProps?.shopaccount?.accountbalance)}{' '}
				</span>
			)}
		</Button>
	);
