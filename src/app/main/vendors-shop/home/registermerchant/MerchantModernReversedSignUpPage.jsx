...
function MerchantModernReversedSignUpPage() {
	...
	const { data: plan, isLoading: isLoadingPlan, isError: isErrorPlan } = useSingleShopplans(accountId);
	...
	// Build referral query params based on referral type
	const buildReferralParams = () => {
		const referralCode = getValues()?.referralCode;
		const referralType = getValues()?.referralType;

		if (!referralCode || !referralType) return {};

		const params = {};

		if (referralType === 'admin') {
			params.adminref = referralCode;
		} else if (referralType === 'merchant') {
			params.merchantref = referralCode;
		} else if (referralType === 'user') {
			params.usersref = referralCode;
		}
	...
	function onSubmit() {
		if (plan?.data?.merchantPlan?.isInOperation) {
			const referralParams = buildReferralParams();
			const registryWithReferral = { ...shopregistry, referralParams };
			...
			if (images?.length > 0) {
				...
				uploadTask.then((snapshot) => {
					getDownloadURL(snapshot.ref).then((downloadURL) => {
						...
						sigupMerchant.mutate(registryWithReferral);
					});
				});
			} else {
				sigupMerchant.mutate(registryWithReferral);
	...
	const resendOTP = () => {
		if (!clientSignUpData) {
			if (window.confirm('Some hitch occurred, restart the onboarding process?')) {
				removeMerchantSignUpToken();
				removeResendMerchantSignUpOtp();
			}
		}
	...
	}, [location?.id, businezState?.id, businezLga?.id, sigupMerchant?.isSuccess]);
	...
	}, [plan?.data?.merchantPlan?.plankey, sigupMerchant?.isSuccess, remoteResponseToken]);
	...
	let bodyContent = (
		...
		>
			...
			{/* Referral Code Section - Subtle and Optional */}
			<Box className="mt-8">
				<Button
					size="small"
					variant="text"
					...
					startIcon={
						<FuseSvgIcon size={16}>
							{showReferralField ? 'heroicons-outline:chevron-up' : 'heroicons-outline:gift'}
						</FuseSvgIcon>
					}
				>
				...
				<Collapse in={showReferralField}>
					<Box
						...
						sx={{ backgroundColor: alpha('#FF6B35', 0.03), border: `1px solid ${alpha('#FF6B35', 0.1)}` }}
					>
						<Typography
							className="text-12 mb-12"
							sx={{ color: alpha('#000', 0.6) }}
						>
							Enter a referral code to earn rewards for both you and the referrer
						</Typography>
						...
						<Controller
							...
							render={({ field }) => (
								<FormControl
									...
									className="mb-16"
								>
									<FormLabel sx={{ fontSize: '12px', mb: 1, color: alpha('#000', 0.7) }}>
										Referral Type
									</FormLabel>
									<Select
										{...field}
									...
									>
										...
										<MenuItem value="admin">Admin Referral</MenuItem>
										<MenuItem value="merchant">Merchant Referral</MenuItem>
										<MenuItem value="user">User Referral</MenuItem>
									</Select>
								</FormControl>
							)}
						/>
			...
			>
				...
				{loading && (
					<Box
						...
						sx={{ backgroundColor: alpha('#FF6B35', 0.1) }}
					>
						<FuseSvgIcon
							size={20}
							sx={{ color: '#FF6B35' }}
		...
		bodyContent = (
			...
			>
				...
				>
					...
					<Box
						className="mt-20 p-16 rounded-lg"
					...
					<div className="relative z-10 w-full max-w-2xl">
						...
						>
							...
							<Box
								...
								sx={{ backgroundColor: 'rgba(255, 255, 255, 0.15)', backdropFilter: 'blur(10px)' }}
							>
								<Chip
								...
								>
									{plan?.data?.merchantPlan?.planinfo ||
										'Join thousands of successful merchants growing their business with us'}
								</Typography>
							</Box>
							...
							<div className="mx-auto w-full max-w-384 sm:mx-0 sm:w-384">
								...
								{plan?.data?.merchantPlan?.isInOperation ? (
									<>
										...
										>
											<AnimatePresence mode="wait">{bodyContent}</AnimatePresence>
											...
											<Box className="mt-40 flex gap-16">
												...
												{step < STEPS.DESCRIPTION ? (
													<Button
														...
														sx={{
															background:
																'linear-gradient(90deg, #FF6B35 0%, #F77F00 100%)',
															color: 'white',
															fontWeight: 700,
															'&:hover': {
																background:
																	'linear-gradient(90deg, #F77F00 0%, #FF6B35 100%)'
															}
														}}
														endIcon={
															<FuseSvgIcon size={20}>
									...
									<Box
										className="mt-32 p-32 rounded-xl text-center"
										sx={{ backgroundColor: alpha('#FF6B35', 0.1) }}
									>
										<FuseSvgIcon
											size={48}
										...
										<Typography className="text-18 font-bold mb-8">Plan Not Available</Typography>
